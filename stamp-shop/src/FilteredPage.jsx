...
function FilteredPage({ filter, onSelectStamp, onBack }) {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/stamps/filter?filter=${encodeURIComponent(filter)}`)
            .then(async (res) => {
                if (!res.ok) {
    ...
    if (!filter) {
        return <div className="text-center py-20 text-slate-400">Please select a filter.</div>;
    }
    return (
            ...
            <div className="max-w-[1200px] mx-auto">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-24 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500 font-light">No stamps found for "{filter}".</p>
                    </div>
                ) : (
