...
function SearchPage({ query, onSelectStamp, onBack }) {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/stamps/search?query=${encodeURIComponent(query)}`)
            .then(async (res) => {
                if (!res.ok) {
    ...
    if (!query) {
        return <div className="text-center py-20 text-slate-400">Please enter a search term.</div>;
    }
    return (
            ...
            <div className="max-w-[1200px] mx-auto">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-24 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500 font-light">No stamps found for "{query}".</p>
                    </div>
                ) : (
